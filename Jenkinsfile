pipeline {
  ...
  environment {
    DOCKERHUB_CREDENTIALS = durga852
    REMOTE_SERVER = '13.53.188.30/8080'
    REMOTE_USER = '16.170.15.54/22' 	  	  
  }
  ...
  stages {
    stage('checkout') {
      steps {
        git branch: 'main', url: 'https://github.com/Prasad852/javawebapp.git'
  ...
  // Test Java application
   ...
   // Push image to DockerHub registry
    ...
    stage('Push Image to dockerHUb') {
      steps {
        sh 'docker push durga852/javawebapp:latest'
      }
      post {
        always {
          sh 'docker logout'
   ...
   // Pull docker image from DockerHub and run in EC2 instance 
    ...
    stage('Deploy Docker image to AWS instance') {
	  ...
	  sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker pull palakbhawsar/javawebapp'"
          sh "ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${REMOTE_SERVER} 'docker run --name javaApp -d -p 8081:8081 durga852/javawebapp'"
          }
        }
      }
    }
